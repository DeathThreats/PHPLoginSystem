...
<body>
            ...
            <ul class="navbar-nav">
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-items" aria-labelledby="navbarDropdown">
                        ...
                        <a class="dropdown-item" href="#" data-toggle="tooltip" data-placement="bottom" title="Account Settings"><svg width="1.3em" height="1.3em" viewBox="0 0 16 16" class="bi bi-envelope-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M.05 3.555A2 2 0 0 1 2 2h12a2 2 0 0 1 1.95 1.555L8 8.414.05 3.555zM0 4.697v7.104l5.803-3.558L0 4.697zM6.761 8.83l-6.57 4.027A2 2 0 0 0 2 14h12a2 2 0 0 0 1.808-1.144l-6.57-4.027L8 9.586l-1.239-.757zm3.436-.586L16 11.801V4.697l-5.803 3.546z"/></svg> Get Help</a>
    ...
    <div class="content container" style="padding-top:1.3rem">
        <h2 class="text-success mb-2">Showing Results for:  <ins class="text-dark font-weight-bold"><?php echo $_POST['search']; ?></ins></h2>
        <div class="table-responsive-xl">
            <table class="table mt-1 table-hover table-striped text-nowrap text-center">
                <thead>
                    <tr class="bg-info">
                ...
                <?php
                    if(isset($_POST['search-submit'])) {
                        ...
                        $searchInput = trim(mysqli_real_escape_string($conn, $_POST['search']));
                        $sql = "SELECT * FROM users WHERE user_id LIKE '%$searchInput%' OR user_fullname LIKE '%$searchInput%' OR user_name LIKE '%$searchInput%' OR date_created LIKE '%$searchInput%' OR user_privilege LIKE '%$searchInput%'";
                        $result = mysqli_query($conn, $sql);
                        $queriedResult = mysqli_num_rows($result);
                        if($queriedResult > 0){

                            while($row = mysqli_fetch_assoc($result)){
                                $row['date_created'] = substr($row['date_created'], 0, 11);
                                echo '<tr><th scope="row">'.$row["user_id"].'</th><td>'.$row["user_fullname"].'</td><td>'.$row["user_name"].'</td><td>'.$row["date_created"].'</td><td>'.ucfirst($row['user_privilege']).'</td></tr>';
                            }
        ...
        <div class="fixed-bottom container d-flex align-center justify-content-center">
            <a href="../index.php#v-pills-developer" class="btn btn-lg btn-dark">Return to Homepage</a>
        </div>
    </div>
</body>
</html>